    ...
    return (
        <div className=" w-full md:w-1/2 flex flex-col gap-6 p-5 md:px-0 mr-5">
            <div className="flex flex-col gap-3">
                <div className="w-fit text-black inline text-[12px]  font-400 px-2 py-[6px] leading-3 rounded" style={{ background: `${backgroundColor}` }}>
                ...
                <div className="flex items-end gap-2 pb-6">
                    <p className="leading-none font-600  text-2xl md:text-4xl text-[#11998E] font-semibold">
                        ₹ <span>{(product.price - (product.discount / 100) * product.price).toFixed(2)}</span>
                    </p>
            ...
            <div>
                ...
                <div className="flex flex-col md:flex-row items-center justify-between h-auto md:h-[69px] py-5 border-y border-slate-300 ">
                    ...
                    <div className="flex flex-wrap items-center gap-2 mt-3 md:mt-0 ">
                        {product.sizes.map((item, index) => (
                            <div
                                key={index}
                                className={`border px-3 py-1 rounded-md cursor-pointer text-xs ${selectedSize === item.size
                ...
                <div className="flex flex-col md:flex-row items-center justify-between h-auto md:h-[69px] py-5 border-b border-slate-300">
                    ...
                    <div className="flex flex-wrap items-center gap-2 mt-3 md:mt-0">
                        {selectedSize ? product.sizes.find(item => item.size === selectedSize)?.colours.map((color, idx) => (
                            <div className={`flex flex-row border border-black px-3 py-1 rounded-md justify-center items-center gap-2
                            ...
                            product.sizes.map((size) =>
                                size.colours.map((color, idx) => (
                                    <div
                                        key={idx}
                                        className={`flex flex-row border px-3 py-1 rounded-md cursor-pointer items-center gap-2 ${selectedColor === color.colour.name
                                            ? 'border-[#52057B] text-[#52057B] bg-[#F0E5FF]'
                                            : 'border-black text-black'
                                            }`}
                ...
                <div className=" h-auto py-3 border-b border-slate-300 ">
                    <div className="text-[#828282] font-400 text-[14px] leading-4 font-semibold mb-2">Product Detail:</div>
